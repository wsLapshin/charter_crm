...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
    ...
    /**
     * Отдельно получить поля дата оплаты счета и галочки об оплате водителю
     * для анализа и раскраски
     */
    public function getFieldsForPainting($listEntries) {
        $db = PearDatabase::getInstance();

        $whereClause = '';
        foreach ($listEntries as $entry) {
            $rawData = $entry->getRawData();
            $whereClause .= $rawData['salesorderid'] . ', ';
        }
        $whereClause = '(' . substr($whereClause, 0, -2) . ')';
        $query = 'select vtiger_salesorder.salesorderid, cf_819, cf_892, cf_897 from vtiger_salesorder
                  inner join vtiger_salesordercf on vtiger_salesorder.salesorderid = vtiger_salesordercf.salesorderid
                  where vtiger_salesorder.salesorderid in ' . $whereClause;
        ...
        foreach ( $rows as $row ) {
            $rowsOutput[$row['salesorderid']] = $row;
        }
