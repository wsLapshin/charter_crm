	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
        ...
        if ($moduleName == 'SalesOrder' ) {
            ...
            if ( in_array ('cf_773', $filterFields) || in_array('cf_791', $filterFields) ) {
                ...
                foreach ($this->listViewEntries as $key => &$entry) {
                    ...
                    if (!empty($billPayedAt)) {
                        $rawData['colored_bill']='rgb(73, 215, 73)'; //нам оплатили
                    } else {
                        $rawData['colored_bill']=''; //нам не оплатили
                    }
                    
                    if ($driverPayed=='1') {
                        $rawData['colored_driver']='rgb(249, 81, 81)'; //если оплатили
                    } elseif ($driverQueue=='1') {
                        $rawData['colored_driver']='yellow'; //поставлен в очередь на оплату
                    } else {
                        $rawData['colored_driver']='white'; //должны
                    }
             ...
             */
             ...
             $fieldsWidth = array (
                    'bill_street' => 120, //адрес загрузки
                    'ship_street' => 120, //адрес выгрузки
                    'cf_864' => 120, //способ оплаты                   
                    'smownerid' => 120, //ответственный                      
                    'description' => 200, //ответственный                      
             );
