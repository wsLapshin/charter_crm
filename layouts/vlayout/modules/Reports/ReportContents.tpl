...
{strip}
	...
	<div id="reportDetails" class="contents-bottomscroll">
		<div class="bottomscroll-div">
			...
			<table class=" table-bordered table-condensed marginBottom10px" width="100%" style="width:500px;margin:20px auto;">
				<thead>
					<tr class="blockHeader">
                        
						<th>{vtranslate('LBL_FIELD_NAMES',$MODULE)}{$CALCULATION_FIELDS.SalesOrder_Total_SUM}</th>
				...
				{foreach from=$CALCULATION_FIELDS item=CALCULATION_FIELD key=index}
					<tr>
						...
						{foreach from=$CALCULATION_FIELD item=CALCULATION_VALUE key=subindex}
                            {assign var=COLUMN value=substr($subindex, -3, 3)}
                            {if $CALCULATION_VALUE !=''  }
                            <td width="15%">{$CALCULATION_VALUE}</td>
                            {elseif $COLUMNS[$COLUMN] }
                            <td></td>
                            {/if}
						{/foreach}
					</tr>
            ...
            <div class="container">
                <div class="panel panel-default">
                        <table class="table table-bordered">
                                <tr colspan="8" class="blockHeader panel-heading" >
                                    {foreach from=$HEADERS item=HEADER key=NAME}
                                        <td nowrap>{vtranslate($NAME,$MODULE)}</td>
                                    {/foreach}
                                </tr>
                            {foreach from=$DATAGROUPED item=val key=groupval}
                                <tr data-rel="parent" data-groupval="{$groupval}" style="background: grey; color: black; cursor:pointer;">
                                    {foreach from=$val.summary item=summval}
                                        <td>{$summval}</td>
                                    {/foreach}
                                </tr>
                                    {foreach from=$val.rows item=row}
                                <tr data-rel="children_{$groupval}"  style="display:none;">
                                        {foreach from=$row item=rowValue}
                                            <td>{$rowValue}</td>
                                        {/foreach}
                                </tr>
                                    {/foreach}
