...
{strip}
	...
	<td>
		...
		<div>
			...
			<input type="text" id="{$productName}" name="{$productName}" value="{$data.$productName}" class="productName {if $row_no neq 0} autoComplete {/if}" placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}"  {if !empty($data.$productName)} disabled="disabled" {/if}/>
			<!--<input type="hidden" id="{$hdnProductId}" name="{$hdnProductId}" value="{$data.$hdnProductId}" class="selectedModuleId"/>-->
			<input type="hidden" id="{$hdnProductId}" name="{$hdnProductId}" value="13" class="selectedModuleId"/>
			<!--<input type="hidden" id="lineItemType{$row_no}" name="lineItemType{$row_no}" value="{$entityType}" class="lineItemType"/>-->
			<input type="hidden" id="lineItemType{$row_no}" name="lineItemType{$row_no}" value="Services" class="lineItemType"/>
			{if $row_no eq 0}
